//
//  OrdersViewModel.swift
//  TexturaStore
//
//  Created by Matvei Khlestov on 12.03.2026.
//

import Foundation
import Combine
import UIKit

/// ViewModel `OrdersViewModel` для экрана истории заказов.
///
/// Основные задачи:
/// - Наблюдение за списком заказов через `OrdersRepository`;
/// - Форматирование стоимости товаров через `PriceFormattingProtocol`;
/// - Предоставление данных для отображения заказов и их содержимого в UI;
/// - Обеспечение реактивного обновления интерфейса при изменении данных.
///
/// Состояние не сохраняется между сессиями — данные заказов
/// запрашиваются и обновляются в реальном времени через Combine.

final class OrdersViewModel: OrdersViewModelProtocol {

    // MARK: - Deps
    
    private let repository: OrdersRepository
    private let priceFormatter: PriceFormattingProtocol

    // MARK: - State
    
    @Published private(set) var orders: [OrderEntity] = []
    
    var ordersPublisher: AnyPublisher<[OrderEntity], Never> {
        $orders.eraseToAnyPublisher()
    }
    private var bag = Set<AnyCancellable>()

    // MARK: - Init
    
    init(
        repository: OrdersRepository,
        priceFormatter: PriceFormattingProtocol
    ) {
        self.repository = repository
        self.priceFormatter = priceFormatter
        bind()
    }

    private func bind() {
        repository.observeOrders()
            .receive(on: DispatchQueue.main)
            .sink { [weak self] in self?.orders = $0 }
            .store(in: &bag)
    }

    // MARK: - API (для VC)
    
    var sectionsCount: Int { orders.count }

    func rows(in section: Int) -> Int {
        guard orders.indices.contains(section) else { return 0 }
        return orders[section].items.count
    }

    func order(at section: Int) -> OrderEntity? {
        guard orders.indices.contains(section) else { return nil }
        return orders[section]
    }

    func item(section: Int, row: Int) -> OrderItem? {
        guard orders.indices.contains(section) else { return nil }
        let items = orders[section].items
        guard items.indices.contains(row) else { return nil }
        return items[row]
    }
    
    func formattedPrice(_ price: Double) -> String {
        priceFormatter.format(price: price)
    }
}
