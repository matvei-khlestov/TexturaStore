...
///   - товары (c фильтрами по поиску, категориям, брендам, цветам, ценам);
///   - категории;
///   - бренды;
///   - цвета товаров;
/// - предоставление live-потока по конкретному товару (`observeProduct(id:)`);
/// - массовый апсерт DTO → Core Data для продуктов, категорий, брендов и цветов товаров;
...
///   - **NEW**: `observeProducts(query:categoryIds:brandIds:colorIds:minPrice:maxPrice:)` — создаёт отдельный live-stream, ключуется `UUID`;
///   - **legacy**: `observeProducts(query:categoryId:)` — стримы кешируются по составному ключу (поиск+категория);
/// - стримы категорий/брендов/цветов лениво создаются и кешируются (один FRC на тип);
...
final class CoreDataCatalogStore: BaseCoreDataStore, CatalogLocalStore {
    ...
    typealias ProductColorsStreamFactory = (
        _ context: NSManagedObjectContext
    ) -> ProductColorsFRCPublisher
    ...
    private var productColorsStream: ProductColorsFRCPublisher?
    ...
    private var makeProductColorsStream: ProductColorsStreamFactory = { context in
        ProductColorsFRCPublisher(context: context)
    }
    ...
    func _setFactories(
        ...
        productColors: ProductColorsStreamFactory? = nil
    ) {
        if let products { makeProductsStream = products }
        if let categories { makeCategoriesStream = categories }
        if let brands { makeBrandsStream = brands }
        if let productColors { makeProductColorsStream = productColors }
    }
    ...
    func observeProductColors() -> AnyPublisher<[ProductColor], Never> {
        if let s = productColorsStream { return s.publisher() }
        let s = makeProductColorsStream(viewContext)
        productColorsStream = s
#if DEBUG
        print("observeProductColors() create stream")
#endif
        return s.publisher()
    }
    ...
    func upsertProductColors(_ dtos: [ProductColorDTO]) {
        guard !dtos.isEmpty else { return }
#if DEBUG
        let sampleIds = dtos.prefix(6).map(\.id).joined(separator: ",")
        print("upsertProductColors() start count=\(dtos.count) ids[0..5]='\(sampleIds)'")
#endif
        ...
        bg.perform { [weak self] in
            ...
            do {
                try self.genericUpsert(
                    dtos,
                    in: self.bg,
                    fetchRequest: CDProductColor.fetchRequest(),
                    dtoID: { $0.id },
                    entityID: { $0.id },
                    setEntityID: { $0.id = $1 },
                    apply: { $0.apply(dto: $1) },
                    matches: { $0.matches($1) },
                    debugName: "upsertProductColors"
                )
            } catch {
                print("❌ CoreDataCatalogStore: upsertProductColors error: \(error)")
            }
        }
    }
}
    ...
    public func meta(for productId: String) -> ProductMeta? {
        let appLanguage: AppLanguage = {
            if Thread.isMainThread {
                return LocalizationManager.shared.language
            } else {
                var value: AppLanguage = .ru
                DispatchQueue.main.sync {
                    value = LocalizationManager.shared.language
                }
                return value
            }
        }()
        ...
        viewContext.performAndWait {
            ...
            guard let cd = try? viewContext.fetch(req).first else {
                result = nil
                return
            }
            ...
            result = ProductMeta(brandName: brandName, title: title, price: cd.price, imageURL: url)
        }
        return result
    }
}
