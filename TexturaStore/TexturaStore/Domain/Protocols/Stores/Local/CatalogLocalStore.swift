...
///
/// Определяет интерфейс для **локального слоя хранения каталога товаров**, включающего продукты,
/// категории, бренды и цвета товаров.
///
/// Основные задачи:
/// - реактивное наблюдение за изменениями каталога (`observeProducts`, `observeCategories`, `observeBrands`, `observeProductColors`);
/// - фильтрация и поиск товаров по категориям, брендам, цветам и диапазону цен;
/// - синхронизация локального состояния каталога с удалёнными данными (`upsertProducts`, `upsertCategories`, `upsertBrands`, `upsertProductColors`);
...
protocol CatalogLocalStore: AnyObject {
    ...
    ///   - colorIds: Идентификаторы цветов.
    ...
    /// Наблюдает за всеми цветами товаров.
    /// - Returns: Паблишер с массивом `ProductColor`.
    ...
    /// Обновляет или добавляет цвета товаров.
    /// - Parameter dtos: Массив DTO цветов.
